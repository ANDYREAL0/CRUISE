...
namespace Vehicle {
    int PlayerMale, PlayerFemale, Bot1, Bot2, Bot3, Bot4;
...
namespace ItemGroup
{
    int AKM, M416, M16A4, SCAL, G36C, UMP, AUG, Groza, AWM, Mini14, SKS, QBZ, M249, MK47, Mosin;//AR
...
namespace GameData
{
    ...
    std::string GetItemNameByID(int ObjectID)
    {
        if (ItemGroup::AKM == ObjectID)
            ...
            return "QBU";
        else if (ItemGroup::Mosin == ObjectID)
            return "모신나강";
...
#pragma region NameDumper
            ...
            while (1) {
                ...
                if (Gres == "Dacia_A_01_v2_C") {
                    Vehicle::Dacia1 = ia;
                    //cout << Vehicle::Dacia1 << endl;;
                }
                if (Gres == "Dacia_A_02_v2_C") {
                    Vehicle::Dacia2 = ia;
                    //cout << Vehicle::Dacia2 << endl;;
                }
                if (Gres == "Dacia_A_03_v2_C") {
                    Vehicle::Dacia3 = ia;
                    //cout << Vehicle::Dacia3 << endl;;
                }
                if (Gres == "Dacia_A_04_v2_C") {
                    Vehicle::Dacia4 = ia;
                    //cout << Vehicle::Dacia4 << endl;;
                }

                if (Gres == "BP_Scooter_01_A_C") {
                    Vehicle::Scooter1 = ia;
                    //cout << Vehicle::Scooter1 << endl;;
                }
                if (Gres == "BP_Scooter_02_A_C") {
                    Vehicle::Scooter2 = ia;
                    //cout << Vehicle::Scooter2 << endl;;
                }
                if (Gres == "BP_Scooter_03_A_C") {
                    Vehicle::Scooter3 = ia;
                    //cout << Vehicle::Scooter3 << endl;;
                }
                if (Gres == "BP_Scooter_04_A_C") {
                    Vehicle::Scooter4 = ia;
                    //cout << Vehicle::Scooter4 << endl;;
                }
...
#pragma endregion
        ...
        MessageBox(NULL, "Reboot PUBG", "Some Error Occured", MB_ICONERROR);
        exit(1);
        return 0;
    }

    int DecryptCIndex(int input)
    {
        DWORD v13 = _rotl(input ^ 0x2458CB4B, 14);
        DWORD result = v13 ^ (v13 << 16) ^ 0x789A2C7B;

        return result;
    }

    int SpeedVal = 0;
    static int Init_offset;
    void UpdateUData()
    {
        if (read) {
            LocalPlayer = Decrypt_Shield(mem->RVM<uint64_t>(mem->RVM<uint64_t>(UGameInst + __OFFSET__LocalPlayer)));
            ...
            //cout << "CurrentWeapon : " << (LPVOID)LocalPlayer_CurrentWeapon << endl;
            
            //if (!Init_offset) {
            //    for (int i = 0; i < 2000; i += 1) {
            //        if (mem->RVM<float>(LocalPlayer_CurrentWeapon + i) == 1.f || mem->RVM<float>(LocalPlayer_CurrentWeapon + i) == 160.f) {
            //            Init_offset = i;
            //            cout << "InstantHit Offset : " << i << endl;
            //            break;
            //        }
			//    }
			//}
			...
			if (cfg->NoRecoil) {
				cfg->AimSmooth = 1.0f;
				if (mem->RVM<float>(LocalPlayer_CurrentWeapon + 248) == 1.f) {
					mem->WVM<float>(LocalPlayer_CurrentWeapon + 248, 160.f);
				}
			}
			else {
				if (mem->RVM<float>(LocalPlayer_CurrentWeapon + 248) == 160.f) {
					mem->WVM<float>(LocalPlayer_CurrentWeapon + 248, 1.f);
				}
			}
			//}
            //if(mem->RVM<float>(LocalPlayer_CurrentWeapon + 0xF44) != -0.01f)
            //mem->WVM<float>(LocalPlayer_CurrentWeapon + 0xF44, -0.01f);

            //mem->WVM<float>(LocalPlayer_CurrentWeapon + 0xE9C, -9.8f);

            //if (cfg->NoRecoil) {
            //    //cfg->AimSmooth = 1.0f;
            //    if (mem->RVM<float>(LocalPlayer_CurrentWeapon + 340) == 1.f)
            //        mem->WVM<float>(LocalPlayer_CurrentWeapon + 340, 160.f);
            //    //if (mem->RVM<float>(LocalPlayer_CurrentWeapon + 268) == 1.f) {
            //    //    mem->WVM<float>(LocalPlayer_CurrentWeapon + 268, 160.f);
            //    //}
            //}
            //else {
            //    if (mem->RVM<float>(LocalPlayer_CurrentWeapon + 340) == 160.f)
            //        mem->WVM<float>(LocalPlayer_CurrentWeapon + 340, 1.f);
            //    //if (mem->RVM<float>(LocalPlayer_CurrentWeapon + 268) == 160.f) {
            //    //    mem->WVM<float>(LocalPlayer_CurrentWeapon + 268, 1.f);
            //    //}
            //}
            ...
            //cout << "CameraManager : " << (LPVOID)CameraManager << endl;
    ...
    FCameraCacheEntry UpdateSway() {
        //FCameraCacheEntry ReturnCam;
    ...
    FCameraCacheEntry UpdateCamera()
    {
        mytime = getLastRenderTime(LocalPlayer_Pawn);
    ...
    void UpdateActors()
    {
        ...
        for (int i = 0; i < ActorCount; i++)
        {
            ...
            if (CurrentActorPTR != NULL) {
                if (!cfg->DebugMode && ActorID != Vehicle::PlayerFemale && ActorID != Vehicle::PlayerMale &&
                ...
                if (ActorID == Vehicle::PlayerFemale || ActorID == Vehicle::PlayerMale || ActorID == Vehicle::Bot1 || ActorID == Vehicle::Bot2 || ActorID == Vehicle::Bot3 || ActorID == Vehicle::Bot4)
                ...
                //mem->readSTR(mem->RVM<uint64_t>(CurrentActorPTR + __OFFSET__PlayerName), cnames, sizeof(cnames));
